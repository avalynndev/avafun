...
export default function GamePage() {
    ...
    useEffect(() => {
        ...
        const updateGame = (timestamp: number) => {
            ...
            const currentInterval = PIPE_INTERVAL_MIN + Math.random() * (PIPE_INTERVAL_MAX - PIPE_INTERVAL_MIN);
            ...
            pipes.current.forEach(pipe => {
                ...
                ctx.strokeRect(pipe.x, pipe.topHeight + pipe.gap, PIPE_WIDTH, canvas.height - pipe.topHeight - pipe.gap);
            ...
            ctx.beginPath();
            ctx.ellipse(birdX + BIRD_SIZE / 2, birdY.current + BIRD_SIZE / 2, BIRD_SIZE / 2, BIRD_SIZE / 2.5, 0, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.fillStyle = '#000';
            ctx.beginPath();
            ctx.arc(birdX + BIRD_SIZE * 0.6, birdY.current + BIRD_SIZE * 0.4, 4, 0, Math.PI * 2);
            ctx.fill();

            ctx.fillStyle = '#fff';
            ctx.beginPath();
            ctx.arc(birdX + BIRD_SIZE * 0.62, birdY.current + BIRD_SIZE * 0.38, 2, 0, Math.PI * 2);
            ctx.fill();
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div ref={containerRef} className="relative bg-sky-400 rounded-lg overflow-hidden shadow-inner">
                    <canvas
                        ref={canvasRef}
